...
// Type Point : utilisé pour le mode c :
// C'est pour créer une liste vector<Point> pour facilement utilser les données
struct Point {
	double p;
	double pPrime;
...
// Type Range : utilisé pour le mode c :
// Utilisé pour facilement définir la plage [min,max]
struct Range {
	double min;
	double max;
};
...
void construire_passage(int line, int column,
	...
	// On met passage à true car l'appel de cette méthode est fait uniquement
	// si libre est a true. On de vérifie donc pas une 2e fois
