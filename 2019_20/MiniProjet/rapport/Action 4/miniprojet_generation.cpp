...
void calculer_passage(const matrice &libre, matrice &passage);
void construire_passage(int line, int column, const matrice &libre, matrice &passage);
...
int main() {
	...
	// Mode c
		...
		// Nombre de points
		int NBP(102);
		const double step(1. / (NBP - 1));
		double percentageScaler = 1. / nbt;
		double p(0);
		...
		for (int i = 0; i < NBP; i++)
		{
			...
			cout << p << " " << percentageScaler * calcualte_all_grids(libre, passage, n, p, nbt, b, e) << endl;
			p += step;
		}
	} else {
...
void calculer_passage(const matrice &libre, matrice &passage) {
	...
	for (int i = 0; i < columns; i++)
	{
		...
		if(libre[0][i])
		{
			...
			construire_passage(0, i, libre, passage);
		}
	}
}
